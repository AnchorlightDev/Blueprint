...
public class CloseCommand implements SubCommand {
    ...
    public CloseCommand(@NotNull WorldService worldService, @NotNull BlueprintConfig config, @NotNull Logger logger) {
        this.worldService = worldService;
        this.config       = config;
        this.logger       = logger;
    }

    @Override public @NotNull String getName() { return "close"; }
    @Override public @NotNull String getUsage() { return "close <world>"; }
    @Override public @NotNull String getDescription() { return "Unload and close a world"; }
    @Override public @NotNull String getPermission() { return "blueprint.command.close"; }
    ...
    public void execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        try {
            worldService.closeWorld(worldName);
            Messages.send(sender, config, "<gray>World <yellow>'" + worldName + "'<gray> closed.");
        } catch (IllegalArgumentException | IllegalStateException e) {
            Messages.send(sender, config, "<red>" + e.getMessage());
        } catch (StorageException e) {
            logger.severe("[Blueprint] DB error closing world: " + e.getMessage());
